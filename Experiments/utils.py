...
""" def get_CIFAR10(trainsplit:float=1.0,  normalize:bool = True, hflip_prob:float = 0.5):
    data_transforms = init_transform(normalize, hflip_prob)
    trainData = datasets.CIFAR10(root = "Data/CIFAR10", train=True, transform=data_transforms["train transform"], download=True)
    trainData, valData = torch.utils.data.random_split(trainData, calculate_data_split(len(trainData), trainsplit))
    testData = datasets.CIFAR10(root = "Data/CIFAR10", train=False, transform=data_transforms["test transform"], download=True)
    return {"train":trainData, "validation":valData, "test":testData} """
...
def OODMetrics(model, dataloader, ood_dataloader):
    ...
    v1 = mAUPR(confs.cpu(), targets.cpu())
    v2 = mAUC(confs.cpu(), targets.cpu())

    return mAUPR(confs.cpu(), targets.cpu())
...
def evaluate_model(model, dataloader, ood_dataloader):
    OODMetrics(model, dataloader, ood_dataloader)
    return {
...
if __name__ == "__main__":
    ...
    AUPR2(output_probs, labels, n_classes=5, threshold_steps=5, average="macro")
